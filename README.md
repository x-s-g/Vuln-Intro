...
- **Purpose**: Supports automated collection of vulnerability-related datasets（Because the website has updated its anti-crawling system, some crawler files are no longer functional.）
- **Main Files**:
  - `Data_NVD.py`: Web crawler that collects CVE detail page URLs from NVD using specific keywords
  - `Data_URL.py`: Web crawler that accesses each CVE detail page and extracts available patch links

**Usage**:
```bash
# Run NVD data collection
python Data_NVD.py

# Run URL data collection
python Data_URL.py
```
